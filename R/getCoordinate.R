getCoordinate.core <- function(address, city = NULL, output = "json", formatted = F) {
    ...
    if (any(grepl(" |#", address))) 
        warning("address should have blank character!")
    address <- gsub(" |#", "", address)
    
    ### url
    url_head <- paste0("http://api.map.baidu.com/geocoder/v2/?address=", 
        address)
    if (!is.null(city)) 
    ...
    ### result
    result <- tryCatch(getURL(url), error = function(e) {
        getURL(url, timeout = 200)
    })
