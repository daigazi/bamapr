...
#' }
getPlace <- function(place = NULL, city = "北京", page_size = 20, pages = Inf, scope = 1,
                     verbose = TRUE, bounds = NULL) {
        ### character
        ...
        url <- function(page) {
                sprintf("%s%s&output=json&query=%s&page_size=%s&&page_num=%s&scope=%s&region=%s&bounds=%s",
                        url_head, map_ak, place, page_size, page, scope, city, bounds)
        }

        ### formate the result
        formate <- function(x) {
                info <- sapply(x, function(y) c(ifelse(is.null(y$name), NA, y$name), ifelse(is.null(y$address),
                                                                                            NA, y$address), ifelse(is.null(y$location$lat), NA, y$location$lat),
                                                ifelse(is.null(y$location$lng), NA, y$location$lng), ifelse(is.null(y$telephone),
                                                                                                            NA, y$telephone)))
