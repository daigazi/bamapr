...
getBaiduMap <- function(location, width = 400, height = 400, zoom = 10, 
    ...
    if (color == "color") {
        ...
        mapd <- dim(map)
        map <- gray(0.3 * map[, , 1] + 0.59 * map[, , 2] + 0.11 * map[, 
            , 3])
        dim(map) <- mapd[1:2]
    }
    ...
    # map spatial info
    ll <- XY2LatLon(list(lat = lat, lon = lon, zoom = zoom), -width/2 + 
        0.5, -height/2 - 0.5)
    ur <- XY2LatLon(list(lat = lat, lon = lon, zoom = zoom), width/2 + 
        0.5, height/2 - 0.5)
    
    # ll = as.numeric(rev(geoconv(rev(ll)))) ur =
    # as.numeric(rev(geoconv(rev(ur))))
    attr(map, "bb") <- data.frame(ll.lat = ll[1], ll.lon = ll[2], ur.lat = ur[1], 
        ur.lon = ur[2])
